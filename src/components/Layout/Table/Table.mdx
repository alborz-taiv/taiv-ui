...
#### `data: T[]`
An array of data items to display in the table. The type `T` is generic, allowing you to use any data structure. However it extends a type that requires a `key` prop, this can be a string or number.
...
#### `divider?: boolean`
An optional prop that adds a divider between each row. By default it is true.

## <br />

### 💡 Best Practices for ListItem Components

#### 🏗️ Build ListItem Components in Application Repos
...
#### ✅ Use a good key
When building the list, choose a good key prop. This usually involves some unique identifier to object being listed out. Not an index.
...
const data = [
  { key: 'John Doe-1', id: '1', name: 'John Doe', email: 'john@example.com', role: 'Admin' },
  { key: 'John Doe-2', id: '2', name: 'Jane Smith', email: 'jane@example.com', role: 'User' },
];
...
#### 📭 Table with Placeholder (Empty State)

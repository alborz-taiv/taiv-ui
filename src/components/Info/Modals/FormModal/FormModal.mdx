...
Pass a custom `icon` with a `color` to override the variant's color.

<Story of={FormModalStories.WithCustomIconColor} />
...
Control modal size via the `size` prop (CSS value or number in px). Default is `40rem`.

<Story of={FormModalStories.Sizes} />
...
Use `confirmButtonDisabled` to disable the confirm button, e.g. until required form fields are filled. This is useful for validation before allowing submission.

<Story of={FormModalStories.ConfirmButtonDisabled} />

## <br />

### 🔄 Loading Confirm Button

Use `confirmButtonLoading` to show confirm button as loading. Used for loading states.

<Story of={FormModalStories.ConfirmButtonLoading} />
...
5. **Size and responsive behavior**: Use `size` for consistency with your layout. Test at different viewport sizes when using a fixed size.

6. **Use confirmButtonDisabled**: Use `confirmButtonDisabled` for when required fields have yet to be filled, there's an error, etc.
